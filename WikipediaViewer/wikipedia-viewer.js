...
$(document).ready(function () {
    ...
    var filesToLoad = [
        ...
        "https://cdnjs.cloudflare.com/ajax/libs/cytoscape/2.4.4/cytoscape.min.js",
...
function assignObserver(target, config,
    slidesTitleMap) {
    var i = 1;
    var observer = new MutationObserver(
        function (mutations) {
            ...
            if (i % 2 === 0) {
                ...
                $(
                    ".carousel-inner .item .typed-text"
                ).empty();
...
function addTextToSlides(slidesTitleMap) {
    ...
    var textToType = slidesTitleMap[
        indexOfCurentSlide];

    //Show blinker
    var $cursorObj = $obj.find(
        '.cursor');
...
function typeTitle(position, text,
    index, time) {
...
//Generate a page. When done, get info(inluding links, thumnail, etc) from all of the pages that the random page links to.
function getLinksFromPage(parameters) {

    getData(parameters).done(function (
        data, textStatus, xhr) {
        var termObj = data;
        ...
        throw new Error(
            "Error getting the data inside of the populateRandom function"
        );
    });
}
...
function getSubpagesOfSubpages(subnodes,
    parent) {
...
function findAndMapSubterms(searchTerm,
    parentTermObj, numToFind,
    callbackF) {
    if (typeof callbackF ===
        'undefined') {
        callbackF = "";
    ...
    getData(parameters).done(function (
        ...
        if (typeof parentTermObj
            .query !==
            'undefined') { //not a cytoscape obj yet
            parentNode =
                makeNodes(
                    parentTermObj
                )[0];
            ...
            parentNode =
                parentTermObj; //the parent is already a node
        ...
        if (nodes && parentNode) {
            ...
            console.log(
                "FOUND BLANK PAGES. DOING ANOTHER RANDOM SEARCH."
            );
            generateTermWithLinks
                ();
        }
        ...
        throw new Error(
            "Error getting the data inside of the findAndMapSubterms function"
        );
    });
}
...
function makeNodes(data, textStatus,
    xhr) {
...
function addGoogleImage(searchTerm,
    imageReceiver) {
    var pic;
    var imageSearch;
    ...
    google.search.Search.getBranding(
        'branding');
    ...
    function searchComplete() {
        ...
        if (imageSearch.results &&
            ...
            function cb(url, result) {
                if (result ===
                    ...
                    if (q < imageSearch
                        ...
                        console.log(
                            "No more google results. Out of imageSearch.results.length :" +
                            imageSearch
                            .results
                            .length
                        );
            ...
            imageExists(imageSearch.results[
                q].url, cb);
        }
    }
}
...
function countNodesCallback(numOfNodes,
    nodesArr) {
    ...
    if (subArr.length !== 0) {
        var lastToBeFetched = subArr[
            subArr.length - 1];

        //If the number of drawn nodes is < 8 ,keep fetching subnodes.
        if (numOfNodes < 8) {

            for (var i = 0; i < subArr.length -
                1; i++) {
                var linksArr = subArr[i]
                    .data.links;
                var comboSearchTerm =
                    generatedTitleCombo(
                        linksArr);
...
function drawToScale() {
    var boundingH = window.innerWidth -
        10;
    var boundingW = window.innerHeight -
        10;
...
function findConcept(searchTerm) {
    ...
    populateSearchTermResults(
        parameters);

}

function populateSearchTermResults(
    parameters) {
    ...
    getData(parameters).done(function (
        ...
        if (JSON.stringify(data) ===
            ...
            displayErrorMessage
                (parameters.gsrsearch);
            attemptToGuessSearchTerm
                (parameters.gsrsearch);
        } else {
...
function attemptToGuessSearchTerm(
    typedSearchTerm) {
    ...
    getData(parameters).done(function (
        ...
        if (JSON.stringify(data) ===
            ...
            $("#cy").append(
                "Your search \"" +
                searchedText +
                "\" did not match any Wikipedia entries."
            );
        } else {
...
function displayErrorMessage(
    ...
    $message.append("Your search \"" +
        searchedText +
        "\" did not match any Wikipedia entries. Did you mean to search for any of the following?"
    );
    $("#errorM").append($message);
}

function showSearchBoxResults(data,
    textStatus, xhr, showConnections) {
    var nodes = makeNodes(data,
        textStatus, xhr);
    var parent = nodes[0];
    nodes.splice(0, 1);
...
function moveDown() {
    ...
    $("#cy").css("height", window.innerHeight -
        20);
