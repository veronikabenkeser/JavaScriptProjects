...
self.addEventListener('message', function(e) {
	switch (e.data.messageType) {
		...
		case 'record-move':
			recordMove(e.data.oldRow, e.data.oldCol, e.data.newRow, e.data.newCol, e.data.player);
			break;
...
function minimax(depth, player, alpha, beta) {
	...
	if (depth === maxDepth || !availablePos.length || boardContainsWin(playingBoard)[0]) {
		...
		return [result, [oldRow, oldCol], [bestRow, bestColumn]];
...
function findAdjacent(currentPosArr, allEmptySpaces) {
	...
	allEmptySpaces = allEmptySpaces.filter(function(val) {
		...
		return (!(row === val[0] && col === val[1])) && (val[0] === row || val[0] + 1 === row || val[0] - 1 === row) && (val[1] === col || val[1] + 1 === col || val[1] - 1 === col);
	});
